...
const renderPreview = (data) => {
  let newPictureElement = PICTURE_TEMPLATE.cloneNode(true); // клонируем полностью содержание шаблона
...
const renderAllPreviews = (pictures) => {
  let fragment = document.createDocumentFragment(); // создаем "корзину" для готовых изображений
  pictures.forEach((pictureData) => {
    let newPictureElement = renderPreview(pictureData);
